...
idCarro = select("select id_carro from Carro where endereco_mac = '" + str(enderecoMac) + "' ;")
...
def insert_ram(consumo):
    ...
    conexao = criar_conexao_cloud()
    cursor = conexao.cursor()
    sql = "INSERT INTO Medida (horario_registro, fk_dispositivo, valor) VALUES (CURRENT_TIMESTAMP," + str(idDispositivo[0]) + "," + str(consumo) + ");"
    cursor.execute(sql)
...
def insert_proc(dados):
    ...
    if procExiste:
        ...
        idP = select(
            "select id from Processo where pid =" + str(pID) + "and fk_carro = " + str(idCarro[0])+";")
